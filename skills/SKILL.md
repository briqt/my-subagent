...
```json
{
  ...
  "profiles": {
    "default": {
      ...
      "api_key": "sk-your-key",
      "pool": [
        {"name": "model-a", "api_base": "https://other-endpoint", "api_key": "sk-other"},
        "model-b",
        "model-c"
      ],
      "effort": "max"
...
pool 支持两种写法混用：字符串（继承 profile 的 api_base/api_key）或对象（独立端点和密钥）。
...
- stderr：`[task: <task-id>]`，成功后追加 `[feedback] node .../feedback.js <task-id> <score> "<comment>"`
...
验收通过后，**必须执行评分**：dispatch.log 末尾有现成的 feedback 命令，复制执行即可。
