---
name: my-subagent
description: "子代理委派模式 — 通过均衡调度的模型池派发任务。使用前必须阅读本文档理解委派协议。当需要将工作委派给子代理执行时使用：多文件读取分析、代码搜索与修改、方案实现、测试验证、信息整理等超过 2-3 步的工作。也适用于 /dlg、delegate、派发任务、委派子代理、parallel workers 等场景。"
---
...
当任务存在多个独立维度或结论值得交叉验证时，启动并行 worker。

- worker 数量跟随维度数，可适当冗余以换取更高质量
- 覆盖多样性差异化视角
- 重要决策派不少于 3 个独立求解，比较结论一致性
- 简单任务直接做，不强行并行

并行时在同一消息中发送多个 Bash 调用（run_in_background=true），每个调用独立 dispatch：

```bash
# Worker 1
node <skill-dir>/scripts/dispatch.js /tmp/dlg/task/w1_prompt.md --name "w1-analyze" \
  > /tmp/dlg/task/w1_out.md 2> /tmp/dlg/task/w1.log

# Worker 2（同一消息，并行）
node <skill-dir>/scripts/dispatch.js /tmp/dlg/task/w2_prompt.md --name "w2-implement" \
  > /tmp/dlg/task/w2_out.md 2> /tmp/dlg/task/w2.log
```
...
```
请阅读以下分析结果：/tmp/dlg/task/w1_out.md
基于此结果，进一步完成 ...
```
